...
>
> **Status:** ✅ **COMPLETE & verified.** Core CRUD + business rule, full-agentic AI layer
> (every endpoint exposed as a tool), Auth (login + RBAC + 30-min token expiry), `/chat`
> conversation memory, and destructive-action confirmation - all built. This doc is the
> original pre-code sketch; the status markers below are updated to reflect what shipped.
...
| **2 - Operations** | Decide the endpoints / what the system can *do* (the *verbs*). | ✅ done |
| **3 - Build** | Code it: project setup -> create tables -> seed / prepare data -> implement endpoints -> tests. | ✅ done |
| **4 - AI layer** | NL `/chat` -> agent (tool use) wrapping the endpoints; later made **full-agentic** (every endpoint a tool) + **conversation memory** + **confirmation before destructive actions**. | ✅ done |
| **5 - Auth** | Login + roles (admin vs outlet), a layer ON TOP. | ✅ done (RBAC + JWT + 30-min token expiry + per-role `/chat` tools) |
| **Later - LLM extras** | More AI features (Q&A, reschedule suggestions). | 🟡 NL booking + Q&A over live data ✅ built; reschedule suggestions 🔮 still future |
...
> **before** the insert, so a double-booking is rejected and no row is created.
...
> reads are public, writes require a JWT for the matching role. Planned from day one
...
| `role` | `admin_dj` or `admin_outlet` (the "wristband colour") |
...
is asking. (`get_bookings` tool ✅ built - public can call it too, mirroring the read endpoints.)

### Resolved at build time (design-meets-code)
- **How the token is made & verified** - the **JWT** standard (HS256 signed token, 30-min expiry). ✅
- **How each endpoint checks it** - FastAPI's **dependency** pattern (`Depends` + a `require_role` factory). ✅
...
the existing API; it never runs core logic itself.
...
2. **Q&A assistant over the data** ✅ **built** - *"which bookings are on the 11th?"* ->
   the agent calls the read tools, grounded in the live DB (not documents).
3. **Reschedule suggestions** 🔮 still future - when a date is taken: *"Andre is booked
   the 11th but free the 12th & 18th; or DJ Mike is free, similar style."*
